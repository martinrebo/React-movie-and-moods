...
function Movies() {
    ...
    const [genre, setGenre] = useState(12); // genre id default call 12 - Adventure
    useEffect(() => {
    ...
    useEffect(() => {
        // try to pass data to dom. 
        // let text = data.results[1].title;
        // let array = data.results    ;
        // let text2 = array.map();
        // const text = array.map((title) => <li> {title} </li>);
        // console.log(text);

        // document.getElementById('stateData').innerHTML = text;
        // let list = array.map((obj, id) => {
        //     return <div  key={"key" + id}  className="title" id={id} tabIndex="0" >{obj} </div>
        // });
    ...
    let list = data.results.map((title, id) => {

        return <div  key={"key" + title}  className="title" id={title} tabIndex="0" >{title} </div>
    });
    ...
    function apiCall() {
        fetch(urlDiscover)
            ...
            .then(function (myJson) {
                ...
                //     // let movieNameList = document.getElementById("movieNameList");
                // identity.innerHTML = obj;
                //     movieName.innerHTML = myJson.results[0]['title'];
                //     let movieNameList= myJson.results.map((results) => <li>{results.title}</li>);
                // // const titleList = ( myJson.results.map((title) => <li>{title}</li> ) );
            });
    }
